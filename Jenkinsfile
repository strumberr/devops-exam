        ...
        stage('Deploy') {
            steps {
                withCredentials([
                    ...
                    'command -v node >/dev/null 2>&1 || (curl -fsSL https://deb.nodesource.com/setup_24.x | sudo -E bash - && sudo apt install -y nodejs)'
                    ...
                    ssh -o StrictHostKeyChecking=no -i ${FILENAME} ${USERNAME}@target \
                    'cd ~/myapp && nohup node index.js > app.log 2>&1 &'
                    '''
                }
            }
        }
